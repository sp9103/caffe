...
namespace caffe {
	...
	template <typename Dtype>
	void DepthSpatialConcatLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		const int topcount = top[0]->count();
		const int tWidth = bottom[1]->shape()[3];
	...
	template <typename Dtype>
	void DepthSpatialConcatLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
